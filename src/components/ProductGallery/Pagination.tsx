...
import { OnPageChanged } from '../../interfaces'
...
class Pagination extends Component<PaginationProps, PaginationState> {
  ...
  calculatePages() {
    ...
    this.pageLimit = pageLimit ? pageLimit : 30
    this.totalRecords = totalRecords ? totalRecords : 0

    this.pageNeighbours = pageNeighbours ? Math.max(0, Math.min(pageNeighbours, 2)) : 0
        ...
        <ul className="ep-pagination">
          {pages.map((page, index) => {
            if (page === LEFT_PAGE)
              return (
                <li key={index} className="page-item">
                  <Button buttonType="muted" aria-label="Previous" onClick={this.handleMoveLeft}>
            ...
            if (page === RIGHT_PAGE)
              return (
                <li key={index} className="page-item">
                  <Button buttonType="muted" aria-label="Next" onClick={this.handleMoveRight}>
            ...
            return (
              <li key={index} className={`page-item${currentPage === page ? ' active' : ''}`}>
                <Button
                  buttonType={currentPage === page ? 'primary' : 'muted'}
                  size="small"
